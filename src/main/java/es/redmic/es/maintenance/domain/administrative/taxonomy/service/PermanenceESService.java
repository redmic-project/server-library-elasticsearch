...
@Service
public class PermanenceESService extends DomainESService<DomainES, PermanenceDTO> {

	@Autowired
	SpeciesESService speciesESService;

	@Autowired
	public PermanenceESService(PermanenceESRepository repository) {
		super(repository);
	}
	
	@Override
	public void postUpdate(ReferencesES<DomainES> reference) {
		speciesESService.updatePermanence(reference);
	}
}
