...
@Service
public class TrophicRegimeESService extends DomainESService<DomainES, TrophicRegimeDTO> {

	@Autowired
	SpeciesESService speciesESService;

	@Autowired
	public TrophicRegimeESService(TrophicRegimeESRepository repository) {
		super(repository);
	}
	
	@Override
	public void postUpdate(ReferencesES<DomainES> reference) {
		speciesESService.updateTrophicRegime(reference);
	}
}
