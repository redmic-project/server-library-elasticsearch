...
@Service
public class InterestESService extends DomainESService<DomainES, InterestDTO> {

	@Autowired
	SpeciesESService speciesESService;

	@Autowired
	public InterestESService(InterestESRepository repository) {
		super(repository);
	}
	
	@Override
	public void postUpdate(ReferencesES<DomainES> reference) {
		speciesESService.updateInterest(reference);
	}
}
